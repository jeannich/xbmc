...
namespace VIDEO
{
  ...
  {
    CFileItemList items;
    const std::vector<std::string>& regexpExcludeRules = CServiceBroker::GetSettingsComponent()
                                                             ->GetAdvancedSettings()
                                                             ->m_tvshowExcludeFromScanRegExps;
                                           ...
                                           CGUIDialogProgress* pDlgProgress /* = NULL */)
  {
    ...
    for (EPISODELIST::iterator file = files.begin(); file != files.end(); ++file)
    {
      ...
      if (bFound)
      {
        ...
        CFileItem item;
        item.SetPath(file->strPath);
        if (!videoInfoDownloader.GetEpisodeDetails(guide->cScraperUrl, *item.GetVideoInfoTag(), pDlgProgress))
  ...
  std::string CVideoInfoScanner::GetFastHash(
